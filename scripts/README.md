...
**Ingest + real submit (no TTY):** after `npm run build`, run [`local-e2e-submit.sh`](local-e2e-submit.sh) — starts **`@doneisbetter/ingest`** on port **19887**, runs **`impact scan --yes-submit`** with **`IMPACT_SUBMIT_NON_INTERACTIVE=1`**, prints **`/api/stats/overview`**.
